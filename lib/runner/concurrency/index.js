...
class Concurrency extends EventEmitter {
  ...
  runMultipleTestWorkers(modules, args, availColors) {
    ...
    const groupedModules = this.groupModulesIntoGroups(maxWorkerCount, modules);
    ...
    return new Promise((resolve, reject) => {
      Concurrency.buildProcessQueue(maxWorkerCount, groupedModules, (env, modulePath, index, next) => {
        let outputLabel = `child-process-${index}`;
        // Utils.getModuleKey(modulePath, this.settings.src_folders, modules);
        const flags = [...modulePath, '--test-worker'];
  ...
  /**
   * Groups modules on the basis of the workerThreads available
   * @param {Integer} workerCnt 
   * @param {Array} modules
   */
  groupModulesIntoGroups(workerCnt, modules) {

    if (modules.length <= workerCnt) {
      return modules;
    }

    const groupedModules = [];
    while (workerCnt > 0) {
      const modulesPerGroup = Math.ceil(modules.length/workerCnt);
      groupedModules.push(modules.splice(0, modulesPerGroup));
      workerCnt --;
    }
   ...
   * @return {ChildProcess}
   */
  createChildProcess(label, args = [], modules = [], index = 0) {
    this.childProcessOutput[label] = [];
    modules.push('--parallel', '--headless');
    const childArgs = modules;
  ...
  static buildProcessQueue(maxWorkers, modules, fn) {
    // takes one moduleGroup at a time and creates a child process for it.
      ...
      while (workers < maxWorkers) {
        ...
        if (queue.length) {
          ...
          if (Array.isArray(modules)) {
            item.env = modules[0].env;
            item.module = modules.map(module => module.module);
          } else {
            item = modules;
          }
